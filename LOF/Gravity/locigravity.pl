...
#    for(my $a=1;$a<$eps;$a++){
	...
	my %alpha_neighbors;
	    ...
	    foreach my $point (keys %knn)
		     ...
		     printf "doing for point $points[$point][0]";
		     my $count = 0;
			 ...
			 if($dist <= $r){
			     ...
			     $count = $count + 1;
			     $new_neighbors{$point}->{$neighbor} = 1;
                             #$new_neighbors{$point}->{$neighbor} = $dist/$r;
			     printf "$new_neighbors{$point}->{$neighbor}\n"
			     	 
			     
			 }

			 # my $rounded_mdef = $dist;
			 #my $rounded_deviation =  $alpha*$r;

			 #printf "  rounded mdef : $rounded_mdef , rounded_deviation $rounded_deviation";

			 if($dist <= $alpha*$r){
			     
			     printf "alpha neighbor for point $points[$point][0] is $neighbor\n";
			     $alpha_neighbors{$point}->{$neighbor} = $alpha;
			     #$alpha_neighbors{$point}->{$neighbor} = $dist/$r;
	...
	print "\n local_desnsity:  \n";
	     my %local_density; 
			 ...
			 #print "$dist";
			 #if($dist == 0){
			 $counter = $counter + ($G*$m1*$m2)/($dist*$dist);
	   ...
	   my %alpha_density;
	     ...
	     my %average_local_density;
	        ...
	        foreach my $point (keys %knn){
		      ...
		      $average_local_density{$point} = $counter / $total_number;
...
#		    my $rounded_mdef = int($mdef{$point} + $float/abs($float*2));
		   ...
		   #my  $rounded_mdef = sprintf("%.2f", $mdef{$point});
		    #my $rounded_deviation =  sprintf("%.2f", $temp);

		    #print "$rounded_mdef : $rounded_deviation";
...
print " For point $points[9][0] for r $r is $mdef_per_r{$points[9][0]}->{$r} \n";
